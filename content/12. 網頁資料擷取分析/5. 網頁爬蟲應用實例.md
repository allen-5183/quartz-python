...
> 3. 第`8`行:顯示網頁標題。
> 4. 第`10`行: 使用`select()`方法取得`id`為`「ct100_labNews」`的串列物件並指定給`data`。
> 5. 第`0`~`11`行:因為`data[0]`為串列第一個元素,即是新書快報的區域,所以由`data[0]`執行`find_all()`方法取得該元素內所有的`<a>`標籤並指定給`link`串列。
> 6. 第`12`~`13`行:使用迴圈印出所有新書名稱,即印出`link`串列中`<a>`標籤的內容。
...
1. 請連上 http://www.evertop.com.tw/ 網頁,並透過瀏覽覽器檢視此網頁的程式碼,結果發现商品新訊内容置於第`2`個`<select class="mb>`内。
   ![[assets/5. 網頁爬蟲應用實例/IMG-20260724231145384.png]]

2. 如下圖,第`2`個`<select class="mb">`内的`<img>`可取得產品新訊的圖:第`2`個 `<select class="mb">`中`<div class="mtitle">`内的`<a>`可取得產品新訊息標題。
...
>說明
> 第`39`行: 由於此行敘述過長無法排成一行,請讀者實際撰寫程式時將此行敘述撰寫成一行。
