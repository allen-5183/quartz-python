...
| **使用者全域** | 透過指令面板開啟 `MCP: 開啟使用者設定。`<br>`%AppData%/Code/User/mcp.json`  | 所有工作區共用 |
...
![[assets/1. VS Code 環境中使用 MCP MySQL 與 MSSQL 服務/IMG-20260811190216340.png]]

![[assets/1. VS Code 環境中使用 MCP MySQL 與 MSSQL 服務/IMG-20260811190652396.png]]
...
- [[2. Antigravity 環境中使用 MCP MySQL 與 MSSQL 服務]]：同樣主題但改在 Antigravity IDE 環境下設定，可對照兩者差異。
- [[3. GitHub Copilot]]：紀錄了 Copilot Chat 的 MCP 伺服器停用問題，是本篇常見問題排除的延伸案例。
- [[1. 如何在 Antigravity IDE 使用 Claude code AI Agent]]：了解另一種 AI Agent 在 IDE 中的登入與使用方式。
- [[2. 在 Visual Studio Code IDE 開發 Python 程式如何設定除錯]]：同樣涉及 `.vscode` 目錄下設定檔（`launch.json`）的操作技巧。
- [[1. Visual Studio Code 擴充套件推薦]]：熟悉 VS Code 基本擴充套件安裝流程，有助於理解本篇的套件安裝步驟。
