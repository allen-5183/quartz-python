...
1. 第三方套件，需另外安裝，支援 `Python 2` 與 `3` 的多個版本
2. 安裝 `python 3`
...
在 `pyproject.toml` 中，可以指定專案的**依賴**及其**版本**，指定版本的方法有以下幾種常用方法:
...
- [[2. pip 套件管理工具]]：虛擬環境建立後，需要搭配 `pip` 來安裝與管理套件。
- [[4. python 多版本執行]]：搭配 `pyenv` 切換 Python 版本，再建立對應版本的虛擬環境。
- [[1. Visual Studio Code 擴充套件推薦]]：了解如何在 VS Code 中選擇並使用建立好的虛擬環境。
